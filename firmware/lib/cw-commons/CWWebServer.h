...
struct ClockwiseWebServer
{
  ...
  void processRequest(WiFiClient client, String method, String path, String key, String value)
  {
    if (method == "GET" && path == "/") {
      ...
      if (key == ClockwiseParams::getInstance()->PREF_DISPLAY_BRIGHT) {
        ...
        Serial.printf("== 1: %hhd\n", (value == "1"));
        ClockwiseParams::getInstance()->timeControl = (value == "1");
  ...
  void getCurrentSettings(WiFiClient client) {
    ...
    client.printf(HEADER_TEMPLATE_D, ClockwiseParams::getInstance()->PREF_ENABLE_TIME_CONTROL, ClockwiseParams::getInstance()->timeControl);
